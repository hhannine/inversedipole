...
% parp = gcp; %probably not needed for charm
...
charm_xbj_bins = [0.00018, 0.00035, 0.0005, 0.0008, 0.0032];
...
data_name_key = "heraII_CC_filtered";
if use_real_data
    % all_xbj_bins = real_xbj_bins;
    all_xbj_bins = charm_xbj_bins;
end

quark_mass_schemes = [
        "mass_scheme_heracc_charm_only",
    ];
mscheme = quark_mass_schemes(1); % charm_only
...
% lambda_strict = [lam1*17e-2]; % charm nn=5 (too high for 4)
lambda_strict = [lam1*10e-2];
...
nn=4; % this is the most (4) points in a bin for charm.
% nn=5;
% nn=13;
for xi = nn:nn
