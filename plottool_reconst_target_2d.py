...
def main(plotvar="xbj"):
    ...
    if plotvar == "xbj":
        plt.xlabel(r'$x_{\mathrm{Bj.}}$', fontsize=15)
        plt.ylabel(r'$Q (median) ~ \left(GeV\right)$', fontsize=15)
        ax.set_zlabel(r'$\frac{\sigma_0}{2} ~ \left(\mathrm{mb}\right)$', fontsize=15)
        xvar = xbj_bins
    elif plotvar == "W":
    ...
    xvar = np.log10(xvar)
    # yvar = q_averages
    yvar = np.log10(q_averages)

    # ax.scatter(x_srted, yvar, [gev_to_mb*real_sigma]*len(x_srted),
    #         # label=labels[i],
    #         label="Fit sigma0/2",
    #         linestyle=":",
    #         linewidth=lw*1,
    ...
    #         color="black"
    #         )
    ax.scatter(xvar, yvar, gev_to_mb*Nlight_max,
            label="Reconstuction of target transverse area",
            linestyle="", marker="o",
            color=colors[0],
            alpha=1
            )
    ax.errorbar(xvar, yvar, gev_to_mb*Nlight_max, zerr=Nlight_errs,
                linestyle="", marker="",
                linewidth=2.0,
                capsize=4.,
                capthick=1.0,
    ...
    ax.scatter(xvar, yvar, gev_to_mb*Ncharm_max,
            label="Reconstuction of target transverse area",
            linestyle="", marker="s",
            color=colors[1],
            alpha=0.7
            )
    ax.errorbar(xvar, yvar, gev_to_mb*Ncharm_max, zerr=Ncharm_errs,
                linestyle="", marker="",
                linewidth=2.0,
                capsize=4.,
                capthick=1.0,
    ...
    # ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('%.03f'))
