...
def main(use_charm=False, real_data=False, fitname_i=None):
    ...
    # real_sigma = data_list[0]["real_sigma"][0] # Not needed, Nfit should have it?
    ...
    dip_data_fit = [] # For HERA data rec. the reference fit might not exist, so got to be prudent:
    b_ref_dipole = []
    for xbj, dat in zip(xbj_bins, data_list):
        if xbj < 0.01:
            dip_data_fit.append(dat["N_fit"][:,0])
            b_ref_dipole.append(dat["sigmar_ref_dipole"][:,0])
        else:
            # print(dat["r_steps"][0][0])
            a = np.array(dat["r_steps"][0][0] * [0])
            dip_data_fit.append(a)
            # print(xbj_bins.index(xbj), Q2vals_grid[xbj_bins.index(xbj)], len(Q2vals_grid[xbj_bins.index(xbj)]))
            b_ref_dipole.append(len(Q2vals_grid[xbj_bins.index(xbj)]) * [0]) # this perhaps should be an array of [0] * len(Q points in this bin)?
    dip_data_fit = np.array(dip_data_fit)
    # b_ref_dipole = np.array(b_ref_dipole)
    ...
    if not use_real_data:
        ...
        # plt1_xbj_bins = [xbj_bins.index(1.3e-2), xbj_bins.index(1.3e-3), xbj_bins.index(5e-3)]
        plt1_xbj_bins = [xbj_bins.index(8e-2), xbj_bins.index(5e-2), xbj_bins.index(3.2e-2), xbj_bins.index(2e-2), xbj_bins.index(1.3e-2), 
                         xbj_bins.index(8e-3), xbj_bins.index(5e-3), xbj_bins.index(3.2e-3), xbj_bins.index(2e-3), xbj_bins.index(1.3e-3), ]
        print(plt1_xbj_bins)
    for xbj, dat in zip(xbj_bins, data_list):
        # print(dat.keys())
    ...
    binned_b_ref_dipole = [b_ref_dipole[i] for i in plt1_xbj_bins]
    # binned_b_hera = [b_hera[i] for i in plt1_xbj_bins]
    # binned_b_errs = [b_errs[i] for i in plt1_xbj_bins]
    ...
    scalings = [1, 1, 1]
    additives = [0, 2, 4]
    # colors = ["blue", "green", "brown", "orange", "magenta", "red", "pink", "grey", "cyan", "yellow"]
    colors = ["indigo", "blue", "teal", "orchid", "navy", "red", "orange", "goldenrod", "crimson", "coral"] # xkcd colors, last 5 have fits
    ...
    ucert_patches = list(reversed([Patch(facecolor=colors[i], alpha=color_alph) for i in range(len(plt1_xbj_bins))]))
    line_fit0 = Line2D([0,1],[0,1],linestyle='-',linewidth=lw*0.6, color="black")
    ...
    if use_real_data:
        ...
        sigr_marker = ""
        sigr_linestyle = ":"
    for ibin in reversed(plt1_xbj_bins):
    ...
    for i, (b_data, b_rec) in enumerate(zip(binned_b_ref_dipole, binned_b_rec)):
        x_srted, b_data = zip(*sorted(zip(xvar[i], b_data)))
        x_srted, b_rec = zip(*sorted(zip(xvar[i], b_rec)))
        # b_data = np.array(b_data)[:,0]
        if len(x_srted) != len(b_data):
            print("len(x_srted), len(b_data), len(b_rec)")
            print(len(x_srted), len(b_data), len(b_rec))
            print("Need to figure out cuts in Q^2 for the fit calculation!")
        # errs = binned_b_err[i] # no error for fit now
        if b_data[0] != 0:
    ...
    for i, xbj in enumerate(plt1_xbj_bins):        
        ...
        ax.fill_between(x_srted, b_rec_dn, b_rec_up, color=colors[i], alpha=0.28)
        ax.fill_between(x_srted, b_rec_95_dn, b_rec_95_up, color=colors[i], alpha=0.16)
    ...
    if use_real_data:
        if not use_charm:
            n_plot = "prelim-hera_rec-sigmar-lightonly-"
        elif use_charm:
            n_plot = "prelim-hera_rec-sigmar-lpcharm-"
...
# Production plotting
main(use_charm=False,real_data=True,fitname_i=3) # Hera data rec fit comparison
